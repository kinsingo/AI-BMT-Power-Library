...
public:
    ...
    /** @brief Cumulative total energy (Joules) since driver load. Volta+ only. */
    double get_total_energy(int gpu_index) const {
        return power_query_->get_total_energy(gpu_index);
    }

    /** @brief Whether a GPU supports the hardware energy counter. */
    bool supports_energy_counter(int gpu_index) const {
        return power_query_->supports_energy_counter(gpu_index);
    }
